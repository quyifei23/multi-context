...
Options parse(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--entry-wrap") { o.entry_wrap = true; continue; }
...
public:
    ...
    CUresult launch_tiny_only(Sample& s) {
        ...
        s.b_launch_seq = ++launch_sequence_; s.b_launch = now_ns();
        const CUresult result = cuLaunchKernel(tiny_, 1, 1, 1, 1, 1, 1, 0, stream_, args, nullptr);
        s.b_launch_return = now_ns(); return result;
    }
    void verify_last_tiny() {
        // Only after both measured launches and all ring snapshots finish.
        CU(cuEventRecord(end_, stream_)); synchronize_event(end_);
        TinyResult result{}; CU(cuMemcpyDtoH(&result, tiny_output_, sizeof(result)));
        if (result.value != (token_ ^ kTokenMask) || result.end_gpu_ns < result.start_gpu_ns)
            throw std::runtime_error("Invalid last isolated tiny result");
    }
#endif
